...
//------------------------------------------------------------------------
//
// Module Name:     BaseWebSNSExtractor.cpp
//
//------------------------------------------------------------------------
// Notes:
//      This file define the functions of class BaseWebSNSExtractor. 
//------------------------------------------------------------------------
// Change Activities:
...
//-----------------------------------------------------------------------
// Func Name   : BaseWebSNSExtractor
// Description : Constructor.
// Parameter   : void
// Return      : void
//-----------------------------------------------------------------------
BaseWebSNSExtractor::BaseWebSNSExtractor()
{
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::OnSysClosed()
{
    {
...
bool BaseWebSNSExtractor::IsSysClosed()
{
    bool isClosed;
    {
        boost::mutex::scoped_lock lock(sigMut2_); 
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::PushNode(Node* node)
{
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::LoopStore(void* obj)
{
    //assert(obj != NULL);
    if (obj == NULL)
		return;
    // Impress an object of BaseTextExtractor into this thread function.
	BaseWebSNSExtractor* extractor = reinterpret_cast<BaseWebSNSExtractor*>(obj);
    // Do loop.
    while (1) {
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::CheckBuf()
{
    ...
    if (node) {
		...
		if (msgNum_ == 0) 
		{
			...
			if(!file_)
			{
				//cout<<" WEBSNS WRITE XML open file error!"<<endl;
				LOG_ERROR(" WEBSNS WRITE XML open file error!\n");
			}else{
			StoreMsg2Xml(node);
			++msgNum_;
			}
...
void BaseWebSNSExtractor::MoveDataFile()
{
    //char srcPath[256];
    char destPath[256];
    time_t timeVal;
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::RegSessionFunc(SessionProc sessionProc)
{
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::ProcessSession(Node*& node)
{
...
// Return      : void
//-----------------------------------------------------------------------
void BaseWebSNSExtractor::StoreMsg2Xml(Node* node)
{
...
/*
void BaseWebSNSExtractor::StoreMsg2DB(Node* node, u_int clueId)
{
