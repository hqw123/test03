...
unsigned int Occi::DoSqlRetInt(oracle::occi::Statement* stmt)
{
    assert(stmt != NULL);
    unsigned int ret = 0;
    ResultSet* rset;
...
void Occi::SetClobForUTF8(oracle::occi::Statement* stmt, unsigned int parameter)
{
    assert(conn_ != NULL);
    assert(stmt != NULL);
    Clob clob(conn_);
...
unsigned int Occi::UpdateClob(oracle::occi::Statement* stmt, unsigned int parameter, u_char* str, unsigned int strLen)
{
    assert(str != NULL);
    assert(stmt != NULL);
